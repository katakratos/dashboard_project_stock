...
const page = () => {
  ...
  const fetchProducts = async () => {
     ...
     const handleQuantityChange = (productId: string, quantity: number) =>{
        setOrder((prevOrder) => 
            prevOrder.map((item) => item.productId === productId ? {...item, quantity} : item
            )
        )
     }
    ...
    const handleSubmit = async () => {
        ...
        try {
            if(order.length === 0) {
                toast.error("Veuillez ajouter des produits au panier avant de confirmer le don.")
                return
           }
            // Call the function to deduct stock with transaction
  ...
  return (
    <Wrapper>
        <div className='flex md:flex-row flex-col-reverse'>
            ...
            <div className='md:w-2/3 p-4 md:ml-4 mb-4 md:mb-0 h-fit border-base-200 rounded-3xl overflow-x-auto'>
            {order.length > 0 ? (
                <div>
